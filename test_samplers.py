...
def test_samplers():
    ...
    for name, sampler in samplers:
        ...
        try:
            ...
            # Test with filtering
            if isinstance(sampler, DCBSSampler):
                token_with_filter = sampler.sample(logits, filter_tokens=filter_tokens, embedding=embedding)
            else:
    ...
    for name, sampler in [("Greedy", greedy), ("DCBS", dcbs)]:
        ...
        for i in range(3):
            if isinstance(sampler, DCBSSampler):
                token = sampler.sample(logits, filter_tokens=filter_tokens, embedding=embedding)
            else:
