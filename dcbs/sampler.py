...
from .clustering import TokenClusterer, CandidateSelector, KMeansClusterer, TopNCandidateSelector
...
class DCBSSampler(Sampler):
    ...
    def __init__(self, clusterer: TokenClusterer, candidate_selector: CandidateSelector, 
    ...
    @classmethod
    def create_default(cls, k: int = 8, top_n: int = 50, cache_config: Optional[dict] = None):
    ...
    def sample(self, logits: torch.Tensor, context: SamplingContext, filter_tokens: Optional[Set[int]] = None) -> int:
    ...
    def _has_invalid_logits(self, logits: torch.Tensor, candidate_ids: list) -> bool:
        ...
        candidate_logits = logits[candidate_ids]
        return torch.isinf(candidate_logits).all() or torch.isnan(candidate_logits).any()
    
    def _fallback_selection(self, logits: torch.Tensor, filter_tokens: Optional[Set[int]]) -> int:
    ...
    def _dcbs_selection(self, logits: torch.Tensor, candidate_ids: list, 
                       embedding: torch.nn.Embedding, filter_tokens: Optional[Set[int]]) -> int:
        ...
        candidate_embeddings = self._get_cached_embeddings(candidate_ids_tensor, embedding)
    ...
    def _select_from_clusters(self, candidate_ids: list, candidate_probs: torch.Tensor,
                            labels: np.ndarray, filter_tokens: Optional[Set[int]]) -> int:
    ...
    def _group_by_clusters(self, labels: np.ndarray, num_clusters: int) -> List[List[int]]:
    ...
    def _calculate_cluster_probabilities(self, clusters: List[List[int]], 
                                       candidate_probs: torch.Tensor) -> List[float]:
    ...
    def _select_best_from_cluster(self, candidate_ids: list, cluster_token_indices: List[int],
                                candidate_probs: torch.Tensor, filter_tokens: Optional[Set[int]]) -> int:
        ...
        if filter_tokens:
            valid_indices = [
                i for i, token_idx in enumerate(cluster_token_indices)
    ...
    def _get_cached_embeddings(self, token_ids: torch.Tensor, embedding: torch.nn.Embedding) -> torch.Tensor:
