...
from .cache_manager import DCBSCacheManager, CacheConfig
...
class BatchDCBSProcessor:
    ...
    def _parallel_batch_sample(self,
                             logits_batch: torch.Tensor,
                             filter_tokens_batch: List[Optional[set]],
                             context: SamplingContext,
                             k: int,
    ...
    def _sequential_batch_sample(self,
                                logits_batch: torch.Tensor,
                                filter_tokens_batch: List[Optional[set]],
                                context: SamplingContext,
                                k: int,
        ...
        with torch.no_grad():
            for i, (logits, filter_tokens) in enumerate(zip(logits_batch, filter_tokens_batch)):
    ...
    def _optimized_dcbs_selection(self,
                                 logits: torch.Tensor,
                                 candidates: List[int],
        ...
        if filter_tokens:
            valid_indices = [
                ...
                if candidates[i] in filter_tokens
            ]
    ...
    def precompute_embeddings(self, 
                            token_ids: List[int],
                            embedding_layer: torch.nn.Embedding) -> None:
        ...
        for i in range(0, len(token_ids), batch_size):
            ...
            uncached_tokens = [
                token_id for token_id in batch_tokens
...
class MemoryEfficientDCBS:
    ...
    def sample_with_memory_limit(self,
                               logits: torch.Tensor,
                               filter_tokens: Optional[set],
                               context: SamplingContext,
                               k: int = 8,
    ...
    def _compute_adaptive_top_n(self, logits: torch.Tensor, target_top_n: int) -> int:
        ...
        if torch.cuda.is_available():
            ...
            max_tokens_by_memory = free_memory // (memory_per_token * 4)  # Safety factor
    ...
    def _get_embeddings_memory_efficient(self,
                                       candidates: List[int],
        ...
        for i in range(0, len(candidates), max_batch_size):
            ...
            with torch.no_grad():
                ...
                embeddings_list.append(batch_embeddings.cpu())  # Move to CPU to save GPU memory
    ...
    def _select_token_from_clusters(self,
                                  logits: torch.Tensor,
                                  candidates: List[int],
                                  labels: np.ndarray,
