...
def create_arc_charts():
    ...
    with open(summary_path, "w") as f:
        ...
        for method, key, acc, correct, total, time_ms in sorted_data:
            f.write(
                f"| **{method}** | {acc:.2f}% | {correct}/{total} | {time_ms:.1f} |\n"
            )

        f.write(f"\n**Random Baseline:** 25.0% (4-choice multiple choice)\n")
        f.write(
            f"**DCBS Improvement over Random:** +{stats['dcbs']['accuracy']*100 - stats['random']['accuracy']*100:.1f} percentage points\n"
        )
        f.write(
            f"**DCBS vs Greedy:** +{stats['dcbs']['accuracy']*100 - stats['greedy']['accuracy']*100:.1f} percentage points\n"
        )
