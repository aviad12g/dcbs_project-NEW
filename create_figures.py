...
def load_results(csv_path):
    ...
    for combo in param_combos:
        ...
        param_details.append(
            {"top_n": int(top_n), "k": int(k), "p": float(p), "key": combo}
        )

    results["metadata"] = {"param_combinations": param_details}
...
def plot_accuracy_comparison(results, output_dir):
    ...
    plt.text(
        3.5, 50.5, "Random guess (50%)", ha="right", va="bottom", color="r", alpha=0.7
    )
...
def plot_parameter_impact(results, output_dir):
    ...
    for param in param_combinations:
        key = param["key"]
        param_labels.append(
            f"top_n={param['top_n']}, k={param['k']}, p={param['p']:.1f}"
        )
...
def plot_accuracy_vs_time(results, output_dir):
    ...
    for method in methods:
        ...
        accuracy = (
            sum(item["correct"] for item in data) / len(data) * 100 if data else 0
        )
...
def generate_report(results, accuracies, times, output_dir):
    ...
    summary = {"total_examples": len(all_data[methods[0]]), "methods": {}}
    ...
    for method in methods:
        ...
        md_content.append(
            f"| {method} | {stats['accuracy']:.2f}% | {stats['avg_time_ms']:.2f} |"
        )
    ...
    md_content.extend(
        [
            ...
            + (
                ...
                if summary["methods"]["dcbs"]["accuracy"]
                > max(
                    summary["methods"]["greedy"]["accuracy"],
            ...
            + (
                ...
                if max(accuracies) - min(accuracies) < 3
                else "There are significant differences in accuracy between methods, "
