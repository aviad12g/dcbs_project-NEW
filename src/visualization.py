...
class AccuracyVisualizer:
    ...
    def create_accuracy_chart(self, results: Dict, output_path: str, 
        ...
        ax.set_xticklabels([m.title() for m in methods], fontsize=self.config.tick_fontsize)
        ax.set_ylabel('Accuracy (%)', fontsize=self.config.label_fontsize)
        ax.set_title('Accuracy by Sampling Method', fontsize=self.config.title_fontsize, pad=20)
    ...
    def create_detailed_comparison(self, results: Dict, output_path: str) -> None:
        ...
        # Extract timing data
        avg_times = [results['statistics'][method].get('avg_time_ms', 0) for method in methods]
        ...
        if intervals:
            ...
            ax1.errorbar(methods, accuracies, yerr=error_bars, fmt='none', 
                        color='black', alpha=0.6, capsize=5)
        
        ax1.axhline(y=50, color='red', linestyle='--', alpha=0.7, label='Random baseline')
    ...
    def _extract_data(self, results: Dict) -> Tuple[List[str], List[float], List[Tuple[float, float]], List[int]]:
    ...
    def _add_significance_annotations(self, ax, results: Dict, methods: List[str], 
        ...
        for i, method in enumerate(methods):
            ...
            p_value = self._binomial_test(
                method_stats['correct'], method_stats['total'],
                best_stats['correct'], best_stats['total']
            )
