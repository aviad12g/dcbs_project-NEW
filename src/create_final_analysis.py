...
def create_cache_comparison_chart(results, output_dir):
    ...
    if results['cached_test'] is None or results['no_cache_test'] is None:
        print("Cache comparison data not available")
        return
...
def create_detailed_analysis_report(results, output_dir):
    ...
    for rank, (method, stats) in enumerate(sorted_methods, 1):
        ...
        report.append(f"| {rank_indicator} | {method} | {accuracy:.1%} | {correct:,}/{total:,} | {avg_time:.0f}ms | {'Excellent' if rank == 1 else 'Good' if rank <= 2 else 'Fair'} |")
    ...
    if greedy_time > 0 and dcbs_time > 0:
        ...
        if abs(overhead) < 5:
            ...
            report.append(f"- **Assessment**: Minimal overhead acceptable")
...
def main():
    ...
    print("Generating comprehensive analysis with figures")
