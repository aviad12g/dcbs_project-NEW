"""
Chat-based evaluation script using HuggingFace chat templates for chain-of-thought reasoning.

This script uses the new sampler architecture and properly implements chain-of-thought
reasoning using HuggingFace's chat template system.
"""
...
# Import our new sampler classes
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from dcbs import GreedySampler, TopPSampler, RandomSampler, DCBSSampler

from token_utils import get_answer_token_ids, is_valid_token_prediction
from errors import EvaluationError, DataError, setup_logging, log_exception
from errors import eval_logger as logger
...
class ChatModel:
    ...
    def generate_response(self, messages: List[Dict[str, str]], sampler, max_new_tokens: int = 50) -> str:
...
def create_final_answer_messages(cot_reasoning: str, sentence: str, options: List[str]) -> List[Dict[str, str]]:
...
def process_example_with_cot(example: Dict, chat_model: ChatModel, sampler) -> Dict:
    ...
    try:
        ...
        cot_reasoning = chat_model.generate_response(cot_messages, sampler, max_new_tokens=100)
        ...
        for option in options:
            ...
            token_ids_with_space = get_answer_token_ids(f" {option}", chat_model.tokenizer, add_leading_space=False)
            token_ids_no_space = get_answer_token_ids(option, chat_model.tokenizer, add_leading_space=False)
...
def evaluate_with_sampler(result: Dict, chat_model: ChatModel, sampler, sampler_name: str) -> Dict:
...
def main(args):
    ...
    try:
        ...
        # Results storage
        all_results = []
        method_stats = {name: {"correct": 0, "total": 0, "avg_time": 0.0} 
        ...
        for i, example in enumerate(benchmark_data):
            ...
            try:
                ...
                for sampler_name, sampler in samplers.items():
                    logger.debug(f"Processing example {example.get('id', i)} with {sampler_name}")
                    
                    # Process example with CoT
                    processed_result = process_example_with_cot(example, chat_model, sampler)
                    
                    # Evaluate with sampler
                    eval_result = evaluate_with_sampler(processed_result, chat_model, sampler, sampler_name)
        ...
        for sampler_name, stats in method_stats.items():
            if stats["total"] > 0:
                ...
                logger.info(f"{sampler_name}: {accuracy:.2f}% accuracy ({stats['correct']}/{stats['total']}), "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chat-based evaluation with chain-of-thought")
    parser.add_argument("--model", type=str, required=True, help="Model name or path")
    parser.add_argument("--benchmark", type=str, required=True, help="Benchmark JSON file")
    parser.add_argument("--output", type=str, required=True, help="Output JSON file")
    parser.add_argument("--top-p", type=float, default=0.9, help="Top-p value for nucleus sampling")
