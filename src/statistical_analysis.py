...
class AdvancedStatistics:
    ...
    @staticmethod
    def mcnemar_test(method1_results: List[bool], method2_results: List[bool]) -> StatisticalTest:
        """
        McNemar's test for paired binary outcomes.
        More appropriate than chi-square for comparing dependent samples.
        """
        # Create contingency table
        both_correct = sum(1 for a, b in zip(method1_results, method2_results) if a and b)
        method1_only = sum(1 for a, b in zip(method1_results, method2_results) if a and not b)
        method2_only = sum(1 for a, b in zip(method1_results, method2_results) if not a and b)
        both_wrong = sum(1 for a, b in zip(method1_results, method2_results) if not a and not b)
    ...
    @staticmethod
    def bootstrap_confidence_interval(data: List[float], n_bootstrap: int = 10000, 
...
def comprehensive_statistical_analysis(results: Dict) -> Dict:
    ...
    significant_differences = [
        key for key, comp in analysis['pairwise_comparisons'].items()
