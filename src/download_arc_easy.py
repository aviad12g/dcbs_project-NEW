...
def create_sample_dataset(filepath, num_items=None):
    ...
    for i, (question, choices, answer) in enumerate(base_questions):
        item = {
            ...
            "question": question,
            "choices": {"text": choices, "label": ["A", "B", "C", "D"]},
            "answerKey": answer,
        }
...
def process_arc_item(item):
    ...
    try:
        ...
        print(
            f"Warning: Answer key '{answer_key}' not found in choices for item {item.get('id', 'unknown')}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and prepare ARC-Easy dataset"
    )
    parser.add_argument(
        "--data_dir", type=str, default="data", help="Directory to save data"
    )
    parser.add_argument(
        "--limit", type=int, help="Limit number of examples (for testing)"
    )
    parser.add_argument(
        "--use_sample",
        action="store_true",
