...
import torch
import psutil
from typing import Dict, List, Optional, Set, Any, Union, TypedDict, NamedTuple
from load_model import load_model_and_tokenizer
from dcbs import category_sample, DCBSConfig, CacheConfig
...
from token_utils import tokenizer_cache
...
def handle_multitoken_answer(
    ...
    logger.debug(f"Answer '{choice_text}' tokenization: '{tokenizer.decode(tokens)}' -> {tokens}")
    ...
    if strategy == "first":
        # Use the first token
        logger.debug(f"Using first token strategy for '{choice_text}': {tokens[0]} -> '{tokenizer.decode([tokens[0]])}'")
        return tokens[0]
        ...
        logger.debug(f"Using most_likely strategy (fallback to first) for '{choice_text}': {tokens[0]}")
        return tokens[0]
    elif strategy == "combine":
        # Return all tokens for combined handling during validation
        logger.debug(f"Using combine strategy for '{choice_text}': keeping all {len(tokens)} tokens")
        return tokens
    else:
        raise EvaluationError(f"Invalid multi-token strategy: {strategy}", 
                              details={"valid_strategies": ["first", "most_likely", "combine"],
                                      "tokens": tokens,
    ...
    try:
        ...
        if "option1" in example and "option2" in example:
            ...
            for choice in choices:
                ...
                logger.debug(f"Choice '{choice}' tokenized as: {token_ids} -> '{tokenizer.decode(token_ids)}'")
        ...
        with torch.no_grad():
            ...
            original_case_answer = result["normalized_answer_map"].get(normalized_correct)
            ...
            if correct_id is None:
                ...
                choice_details = {
                    choice: {
                        "id": id_val,
                        "decoded": tokenizer.decode([id_val]) if not isinstance(id_val, list) 
...
def evaluate_methods(
    ...
    for method in ["greedy", "top-p", "dcbs", "random"]:
        ...
        if method == "greedy":
            ...
            # Use configured temperature from dcbs_config (or default)
            pred_id = category_sample(
                logits[0],
...
def main(args):
    ...
    try:
        ...
        logger.info(f"Logging configured with level {log_level}" + (f" to file: {log_file}" if log_file else ""))
        ...
        batch_size = memory_config.get("batch_size", 5)  # Process examples in batches to limit memory growth
        ...
        tokenizer_cache.report_interval = tokenizer_cache_config.get("report_interval_sec", 60)
        ...
        try:
            ...
            model.config.tokenizer = tokenizer  # Attach tokenizer to model config for convenience
                ...
                for k in k_values:
                    for p in p_values:
                        ...
                        for batch_start in range(0, len(benchmark), batch_size):
                            ...
                            logger.debug(f"Processing batch {batch_num}, examples {batch_start+1}-{batch_end} of {len(benchmark)}")
                            
                            batch = benchmark[batch_start:batch_end]
                            for idx_in_batch, example in enumerate(batch):
                                idx = batch_start + idx_in_batch  # Global index in the benchmark
                                ...
                                try:
                                    ...
                                    for method, res in method_results.items():
                                        ...
                                        evaluation_stats["avg_time_ms"][method] += res["elapsed_ms"]
                        ...
                        accuracies = {}
                        for method in ["greedy", "top-p", "dcbs", "random"]:
                            avg_time = evaluation_stats["avg_time_ms"][method] / (success_count or 1)
                            accuracies[method] = evaluation_stats["correct"][method] / (success_count or 1) * 100
        ...
        logger.info(f"Final statistics: {evaluation_stats['success']} successful, "
