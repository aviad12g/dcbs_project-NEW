...
from dcbs import GreedySampler, TopPSampler, RandomSampler, DCBSSampler, SamplingContext
...
class ChatTemplateManager:
    ...
    def setup_chat_template(cls, tokenizer, model_name: str) -> None:
        ...
        if "llama" in model_name.lower():
            template = cls.TEMPLATES["llama"]
            logger.info("Applied Llama chat template")
        elif "mistral" in model_name.lower():
            template = cls.TEMPLATES["mistral"]
            logger.info("Applied Mistral chat template")
        else:
            template = cls.TEMPLATES["generic"]
            logger.info("Applied generic chat template")
...
class ExampleProcessor:
    ...
    def create_prompt(self, sentence: str, options: List[str], include_cot: bool = True) -> str:
    ...
    def process_example(self, example: Dict, include_cot: bool = True) -> Dict:
        ...
        if include_cot:
            ...
            final_prompt = self.create_final_answer_prompt(sentence, options, cot_reasoning)
        else:
    ...
    def create_cot_prompt(self, sentence: str, options: List[str]) -> str:
        ...
        system_msg = "You are a helpful assistant. Think step by step about the problem."
    ...
    def create_final_answer_prompt(self, sentence: str, options: List[str], reasoning: str) -> str:
    ...
    def generate_reasoning(self, prompt: str, max_length: int = 200) -> str:
        ...
        stop_patterns = ["the answer is", "answer:", "therefore", "so the answer", "option"]
...
class EvaluationRunner:
    ...
    def __init__(self, config: EvaluationConfig):
        self.config = config
        self.model_manager = ModelManager(config.model_name)
        ...
        for i, example in enumerate(benchmark_data):
            ...
            try:
                ...
                for method_name, sampler in self.samplers.items():
                    ...
                    # Process the example
                    processed = processor.process_example(example, self.config.include_cot)
    ...
    def _evaluate_with_sampler(self, processed_example: Dict, sampler, sampler_name: str, 
                             context: SamplingContext, tokenizer) -> EvaluationResult:
        ...
        correct = is_valid_token_prediction(pred_id, correct_id, correct_answer, tokenizer)
    ...
    def _compile_results(self, all_results: List[EvaluationResult], total_examples: int) -> Dict:
        ...
        for method, stats in self.stats.items():
            if stats["total"] > 0:
                ...
                logger.info(f"{method}: {accuracy:.2f}% ({stats['correct']}/{stats['total']})")
    ...
    @staticmethod
    def _calculate_confidence_interval(correct: int, total: int, confidence: float = 0.95) -> Tuple[float, float]:
        ...
        margin = z * np.sqrt((p * (1 - p) + z_squared / (4 * n)) / n) / (1 + z_squared / n)
...
def load_benchmark_data(benchmark_path: str) -> List[Dict]:
    ...
    try:
        ...
        # Validate first example has required fields
        required_fields = ["sentence", "option1", "option2"]
        if not all(field in data[0] for field in required_fields):
            raise DataError(f"Benchmark examples must contain fields: {required_fields}")
