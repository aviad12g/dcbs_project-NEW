...
import torch
from typing import List, Set, Dict, Optional, Tuple, Union, Any
import random
import logging
...
def is_valid_token_prediction(
    ...
    if isinstance(correct_id, list):
        ...
        full_correct_text = tokenizer_cache.decode(correct_id, tokenizer).lower().strip()
...
def get_answer_token_ids(text: str, tokenizer, add_leading_space: bool = True) -> List[int]:
