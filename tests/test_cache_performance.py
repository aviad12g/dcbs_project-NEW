"""
Performance tests to verify that caching actually improves performance.

This module tests whether the embedding and clustering caches provide
measurable performance improvements over uncached operations.
"""
...
from dcbs import DCBSSampler, SamplingContext, KMeansClusterer, TopNCandidateSelector
from dcbs.cache_manager import CacheConfig, reset_cache_manager
...
class TestCachePerformance:
    ...
    def test_embedding_cache_performance(self):
        ...
        cached_sampler = DCBSSampler(clusterer, candidate_selector, cache_config.__dict__)
        uncached_sampler = DCBSSampler(clusterer, candidate_selector, no_cache_config.__dict__)
    ...
    def test_cache_hit_rates(self):
        ...
        if embedding_metrics:
            ...
            assert embedding_hit_rate > 0.5, f"Embedding hit rate {embedding_hit_rate:.2%} too low"
        ...
        if cluster_metrics:
            ...
            assert cluster_hit_rate > 0.7, f"Cluster hit rate {cluster_hit_rate:.2%} too low"
    ...
    def test_cache_vs_no_cache_correctness(self):
        ...
        for i in range(5):
            ...
            cached_result = cached_sampler.sample(test_logits, self.filter_tokens, self.context)
            uncached_result = uncached_sampler.sample(test_logits, self.filter_tokens, self.context)
