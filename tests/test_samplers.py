...
from dcbs import (
    Sampler, GreedySampler, TopPSampler, RandomSampler, DCBSSampler, 
    SamplingContext
)
...
class TestTopPSampler:
    ...
    def setup_method(self):
        ...
        self.logits = torch.tensor([3.0, 2.0, 1.0, 0.0, -1.0])  # Sorted for easy calculation
        ...
        for name, sampler in self.samplers.items():
            ...
            if name == "dcbs":
                token_id = sampler.sample(
                    self.logits, 
                    filter_tokens=self.filter_tokens, 
                    context=self.context
                )
            else:
                token_id = sampler.sample(
...
class TestStatisticalProperties:
    ...
    def setup_method(self):
        ...
        self.biased_logits = torch.tensor([5.0, 1.0, 2.0, 0.5])  # Heavily biased toward index 0
