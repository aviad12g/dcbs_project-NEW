...
from src.run_dcbs_eval import evaluate_methods, process_example
...
class PerformanceBenchmark(unittest.TestCase):
    ...
    def test_batch_processing_memory_usage(self):
        ...
        for batch_size in batch_sizes:
            ...
            logger.info(
                f"Batch size {batch_size}: {memory_usage[batch_size]:.2f} MB/example, "
        ...
        if memory_usage[1] > memory_usage[5]:
            ...
            logger.info(
                "Note: Batch processing did not reduce per-example memory usage in this environment"
            )
    ...
    def test_tokenizer_cache_performance(self):
        ...
        logger.info(
            f"Cache hit rate: {tokenizer_cache.hits/(tokenizer_cache.hits+tokenizer_cache.misses)*100:.1f}%"
        )
